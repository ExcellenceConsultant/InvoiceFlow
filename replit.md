...
- **Category Margins (Profit Module)**: Customer category-wise margin settings stored in `category_margins` table. Supports both global category margins (applies to all products in that category) and per-product category margins. During invoice creation, margin is resolved in priority order: Category+Product margin → Category global margin → Inventory product margin. Margins are stored as dollar amounts per carton (not percentages) and used directly as margin_per_carton on invoices. The "Add Margin" dialog includes two tabs: Inventory Margin (existing per-carton) and Category Margin (dollar-based by customer category).
- **Role-Based Account Management**: Restrictions on delete and inactive operations for `super_admin` and `admin` roles.
- **Sales Order Module**: Full sales order lifecycle management with status tracking (pending → confirmed → converted/cancelled). One-click conversion to AR invoice (copies all line items, auto-assigns next invoice number, updates inventory). Sales orders show source (manual vs API). External API endpoints (`/api/external/sales-orders` GET/POST/PUT) allow third-party apps to push orders in via Bearer token API keys, with upsert-by-externalId for sync. Visible to all users; edit/convert restricted to non-viewers; delete restricted to admin/super_admin.
