...
- **Invoice Management**: Supports both AR (receivable) and AP (payable) invoice types with status tracking, detailed line items, editable discount fields (0% default), and automated total calculations (Subtotal + Freight - Discount). Invoices are displayed in sequential order sorted by invoice number. **Invoice Numbering**: AR invoices use auto-generated sequential numbers (e.g., 1189, 1190, 1191) without prefixes, while AP invoices require manual entry for flexibility with vendor invoice numbers.
